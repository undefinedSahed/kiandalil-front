...
export function WishlistPropertyCard({ wishlistItem, onRemoveFromWishlist, isRemoving }: PropertyCardProps) {
    const { propertyId } = wishlistItem
    const mainImage = propertyId.images?.[0] || "/placeholder.svg?height=200&width=300"
    ...
    return (
        <Card className="group overflow-hidden border border-gray-200 hover:shadow-lg transition-shadow duration-300 relative">
            <CardContent className="p-0">
                ...
                >
                    <Heart
                        className={`h-4 w-4 transition-colors ${isRemoving
                ...
                <Link href={`/property/${propertyId._id}`} className="block">
                    <div className="relative aspect-video overflow-hidden">
                        ...
                        <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm rounded-md px-2 py-1">
                            <span className="text-sm font-semibold text-gray-900">{formatPrice(propertyId.price)}</span>
                        </div>
                    </div>

                    <div className="p-4">
                        <h3 className="font-semibold text-gray-900 mb-1 line-clamp-1">{propertyId.title}</h3>
                        <p className="text-sm text-gray-600 mb-3 line-clamp-1">{propertyId.subtitle}</p>
                        ...
                        {/* Date added */}
                        <p className="text-xs text-gray-400">Added {formatDate(wishlistItem.createdAt)}</p>
                    </div>
                </Link>
            </CardContent>
        </Card>
